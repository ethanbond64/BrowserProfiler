...
export const SettingsView = (props: { settings: Settings, setSettings: any }) => {
    ...
    const updateHeartbeat = (event: any) => {
        let newSettings = props.settings;
        newSettings.setHeartbeat(event.target.value);
        updateSettings(newSettings);
    };

    const updateTimeout = (event: any) => {
        let newSettings = props.settings;
        newSettings.setSleep(event.target.value);
        updateSettings(newSettings);
    };

    const updateSyncMode = (event: any) => {
        let newSettings = props.settings;
        newSettings.setSyncMode(event.target.checked);
        updateSettings(newSettings);
    }

    const updateSyncUrl = (event: any) => {
        let newSettings = props.settings;
        newSettings.setSyncUrl(event.target.value);
        updateSettings(newSettings);
    };
                ...
                <div className="flex justify-center">
                    <div>
                        <input
                            ...
                            defaultChecked={props.settings?.getSyncMode()}
                        />
                        <label
                        ...
                        <input type="number" name="timeout" id="timeout" onBlur={updateTimeout} defaultValue={props.settings?.getSleep()} />
                        <label htmlFor="timeout">Timeout</label>
                        <input type="number" name="heartbeat" id="heartbeat" onBlur={updateHeartbeat} defaultValue={props.settings?.getHeartbeat()} />
                        <label htmlFor="heartbeat">Heartbeat</label>
                        <input type="text" name="url" id="url" className="b-1" onBlur={updateSyncUrl} defaultValue={props.settings?.getSyncUrl()} />
                    </div>
                </div>
